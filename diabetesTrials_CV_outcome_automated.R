...
cvString = c('mace','cv death','fatal stroke','fatal mi','non-fatal mi','non-fatal stroke')
...
design_outcomes_tbl = tbl(src=con,'design_outcomes')
design_outcomes <- design_outcomes_tbl %>% select(nct_id,outcome_type,measure,description) %>% collect()
design_outcomes <- design_outcomes %>% group_by(nct_id) %>% mutate(cvOutcome = case_when(any(str_detect(tolower(measure), pattern = paste(cvString,collapse="|"))) ~ 1,
                                                                                         any(str_detect(tolower(description), pattern = paste(cvString,collapse="|"))) ~ 1,
                                                                                         TRUE ~ 0))

design_outcomes_distinct <-distinct(design_outcomes,nct_id,.keep_all=TRUE) %>% select(nct_id,cvOutcome)
...
# join only for ones with outcome description
joinedTable <- join_all(list(joinedTable,design_outcomes_distinct),by='nct_id',type="inner")
joinedTable <- joinedTable %>% filter(cvOutcome == 1)
...
if (saveData){
  ...
  write.csv(joinedTable,'diabetesTableTotal_CV_outcome_6_1_2021.csv')
